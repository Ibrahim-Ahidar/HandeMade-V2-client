...
export default function VerifySignupCode() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      clearRecovery();
      setAuthenticated(authData.accessToken ?? authData.token, authData.user);
      goAfterAuth(authData.user);
    } catch (err) {
  ...
  return (
    <AuthLayout
      ...
      centered
      footer={
        <Link to="/signup" className="text-xs text-text-secondary transition hover:text-accent sm:text-sm">
          ← Back to signup
        </Link>
      }
    >
